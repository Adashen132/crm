    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $("#saveRemarkBtn").click(function () {
                $.ajax({
                    ...
                    data: {
                        ...
                        "activityId": "${a.id}"
                    },//textarea是标签对，还用val是应为把它当做表单元素处理。detal.jsp使用请求转发，从请求域中获取a，使用el表达式
                    type: "post",
                    dataType: "json",
                    success: function (data) {
                        //data  {"success":true/false,"ar":{市场活动备注的对象}}
                        if (data.success) {
                            //添加成功，先把文本域中的信息清空！
                            $("#remark").val("");
                            alert("添加备注成功！");//得在文本域textarea上方新增一个div
                            var html = "";
                            ...
                            html += '</div>';
                            html += '</div>';
                            $("#remarkDiv").before(html);//在remarkDiv前加入新的div
                        } else {
                            alert("添加备注失败！");
                        }
                    }
            ...
            $("#updateRemarkBtn").click(function () {
                var id = $("#remarkId").val();//id是隐藏域窗口的id
                $.ajax({
        ...
        function showRemarkList() {
            $.ajax({
                ...
                data: {"activityId": "${a.id}"},<!--user中的id对应activity中的activityId，在js代码中写el表达式，得套用字符串-->
                type: "get",
                dataType: "json",
                success: function (data) {
                    //data:[{备注1},{2},{3}]id,noteContent,createTime,createBy,editTime,editBy.editFlag,activityId
                    var html = "";
        ...
        function deleteRemark(id) {
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        alert("删除备注成功");
                        //showRemarkList();这种方法不行，记录使用的是before方法($("#remarkDiv").before(html))，每一次删除之后，页面上都会保留原有的数据
                        //找到需要删除记录的div，将该div删除
                        $("#" + id).remove();
                    } else {
                        alert("删除备注失败");
                    }
...
<div style="position: relative; left: 40px; top: -30px;">
    <div class="page-header">
        <h3>市场活动-${a.name}<small>${a.startDate} ~ 2${a.endDate}</small></h3>
    </div>
